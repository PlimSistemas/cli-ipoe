...
#------------------------------------------------	
	if [ "$1" == "bandwidth" ]; then
		...
		ifname=$(sudo accel-cmd show sessions ifname match username ${username} | sed -n "3p")
...
#------------------------------------------------	
	if [ "$1" == "ping" ]; then	
		...
		ip_address=$(sudo accel-cmd show sessions ip match username ${username} | sed -n "3p")
